...
- `Invoke-DfuCodecRepairPass`: `RecordCodecBuilder.<T>create/mapCodec` type witnesses; record accessors inside balanced `.validate(...)`; mixin `(T)this` â†’ `(T)(Object)this`; strip CheckerFramework `@NonNull`.
...
- `loadWithComponents(CompoundTag, RegistryAccess)` â†’ `TagValueInput.create(ProblemReporter.DISCARDING, ...)` + imports.
...
- Proven: Mel `gradlew build` SUCCESS â†’ `mels_deco-1.2+mc26.2-neoforge.jar` (~13.1MB).
...
- Encoded Mel/MCreator 1.21.4â†’26.2 GUI/API rules into `Invoke-Mcreator1218ToNeoForge262Pass`:
  strip RenderSystem blend/color, `RenderType::guiTextured`â†’`RenderPipelines.GUI_TEXTURED`, `drawString`â†’`text()`, `ClientPacketDistributor.sendToServer`, yield/break cleanup, emissiveRendering 1-arg, GameProfile.id(), Level weather via instanceof, copyTag Optional NBT, item tooltip/hurtEnemy/inventoryTick.
- ExactPrimer: emissiveRendering 3-argâ†’1-arg after hasPostProcess rename.
...
- Registry `Block id not set` Supplierâ†’Function fix; installer embeds payload only.
...
- Core loop: **detect source version** â†’ select cumulative primer transitions/rules â†’ convert to NeoForge 26.2.
...
### Mappings
- Official SRG map from workstation `Minecraft_Mappings/1.20.1` flat TSV (64,225 unique, 0 conflicts) â†’ `lib/Srg1201Official.json`.
- `Srg1201Common.json` only holds SRGs missing from that flat map (guessed overlays were renaming `playLocalSound` â†’ `setBlock`).

### NeoForge 26.2 load
- `DeferredRegister.Blocks.registerBlock` / `Items.registerItem` + `Properties` constructors (`Block/Item id not set`).
- `ForgeSpawnEggItem(entity,â€¦)` â†’ `properties.spawnEgg(entity.get())`.
...
### GeckoLib 5
- Bare geo IDs (`toww_geckolib` â†’ `assets/<mod>/geckolib/models|animations/`).
...
### Proven
- **The Knocker** â€” 26.2 jar, in-game spawn.
- **The One Who Watches** â€” 26.2 jar loads; GeckoLib geo/anim/textures packed; spawn-egg / summon; world-data crash fixed.

## 1.2.6 â€” 2026-08-01

### Networking rewrite actually applies (MOAdecor BATH retest)
- **Critical fix:** v1.2.5 `MESSAGES.forEach â†’ playBidirectional` rewrite regex never matched
...
Proven: **MOAdecor GARDEN 1.21.8.A** â†’ `gradlew build`.
...
- **Fix:** naive `playBidirectional` 3â†’4-arg expansion could mangle
...
Proven: **MOAdecor ELECTRONICS 1.21.8.A** â†’ `gradlew build` after re-apply.
...
### MCreator / NeoForge 1.21.x â†’ 26.2 pass (MOAdecor BATH)
New rewrite pass for decompiled **1.21.8 NeoForge / MCreator** jars (in addition to Forge 1.20.1). Not a separate converter product â€” same Legacy pipeline + extra pass:
- Remove `shouldDisplayFluidOverlay` + old `BlockAndTintGetter` import (method gone from Block)
- `.noCollission()` â†’ `.noCollision()`
- `GuiGraphics` â†’ `GuiGraphicsExtractor`; `renderBg` â†’ `extractBackground`; tooltip/label extract renames
- Final `imageWidth`/`imageHeight` â†’ `super(menu, inv, title, w, h)` (including delayed field assigns)
- `keyPressed(int,int,int)` â†’ `keyPressed(KeyEvent)` (ESC close pattern)
- `.isClientSide` field â†’ `.isClientSide()`
- `net.minecraft.util.Tuple` delayed work queue â†’ `Object[]` holders
- Stub MCreator `ItemHandler.ITEM` / `ItemHandler.ENTITY` capability binds (transfer API is manual)
- `Minecraft.getInstance().screen` â†’ `gui.screen()`
- `registerItem(name, fn, Properties)` â†’ supplier form `() -> properties`
- Payload `StreamCodec<? extends FriendlyByteBuf` â†’ `? super RegistryFriendlyByteBuf`
- **Critical networking:** use 4-arg `playBidirectional(type, codec, handler, handler)` â€” 3-arg leaves client handler null and crashes with `missing client-side handlers`

Proven: **MOAdecor BATH 1.21.8.A** â†’ compile, `gradlew build`, and **client load on NeoForge 26.2.0.32-beta**.
...
- `EntityType.VANILLA_FIELD` â†’ `EntityTypes.VANILLA_FIELD` (+ import)
- Full **ColorCollection** grid for `Items`/`Blocks` (`WHITE_WOOL` â†’ `WOOL.white()`, glazed terracotta, beds, carpets, â€¦)
- `getMainCamera()` â†’ `mainCamera()`
- `Minecraft.getInstance().renderBuffers()` â†’ `gameRenderer.renderBuffers()`
...
- **ModConfigSpec define-before-build pass:** decompiled MCreator configs that call `BUILDER.build()` before `.define(...)` caused  
  `Cannot get config value before spec is built` on player spawn â†’ **Connection lost / Disconnected**.
...
Proven on **Friend** (runtime) and **The Knocker** (NeoForge 1.21.8 jar â†’ 26.2 compile/build).
...
- Clearer docs: conversion success â‰  loadable mod; only install `gradlew build` output jars; never rename the input jar as 26.2.

### API rewrite expansions (26.2)
- `displayClientMessage` â†’ `sendSystemMessage`
- `getLevelData().getSpawnPos()` â†’ `getRespawnData().pos()`
- `getRespawnConfig().pos()/dimension()` â†’ `respawnData()...`
- Broader `entity.getServer()` â†’ `level().getServer()` receivers
- `CommandSourceStack` permission int â†’ `LevelBasedPermissionSet`
