...
1. Read `../legacy-java-converter-vnext/references/knowledge-sources.md` for declared versions and roots.
...
5. Use returned `physical_path` / `source_root` with native file search and reading; pin the exact version directory.
...
- Walk all of `C:\GokuCodexAI\Data` or use `goku-data.db` for repair (legacy/benchmark only).
