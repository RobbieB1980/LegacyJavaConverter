...
2. Add or update **one** shard under `C:\GokuCodexAI\Data\262r\shards\` (or `converter/` for pipeline notes).
3. Add ledger row to `C:\GokuCodexAI\Data\262r\remaps.md` and/or `do-not.md` if it is an anti-pattern.
4. Update `C:\GokuCodexAI\Data\262r\catalog.json` with id + file + summary.
...
8. Refresh the link-only index used by skills:
   regenerate `.agents/skills/repair-failed-262-output/references/262r-shard-index.md` from `catalog.json` (ids/summaries only).
