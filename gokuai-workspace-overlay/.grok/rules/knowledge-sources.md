...
- **Agent search order:** failed-output packet → **`262r` (converter/ + shards/)** → solved cases → compact `primer_changes` → exact **26.2** physical source → invent + write back to `262r` and the converter.
- Prefer compact `primer_changes` ledger/shards for detected-source → 26.2 before opening full primer `index.md` bodies.
- Before API edits, resolve the primer chain for the detected source → 26.2 (ledger first) and build migration evidence so no intermediate delta is skipped.
...
- Do not start repairs by walking all of `C:\gokuai\Data`; open one 262r shard / primer shard at a time.
- Validation builds: skill `validate-destination-build` (JDK 25). Never probe ambient/source Java 8 first.
- MCP **minecraft-knowledge** must be enabled for indexed lookup. `goku-data.db` is legacy/benchmark-only.
- Playbooks: skills `migrate-neoforge-262`, `repair-failed-262-output`, `encode-262r-remap` (not duplicated here).

## Registered source roots

- `completed_grokbuild` -> `H:\GrokBuild_MF\Completed_Projects` (30445 indexed files; available)
- `local` -> `C:\gokuai\Data` (35972 indexed files; available)

## Exact-version entrypoints

- 262-repair knowledge (category `262r`, version 26.2): `C:\gokuai\Data\262r`
- 262r converter notes: `C:\gokuai\Data\262r\converter`
- 262r remap shards: `C:\gokuai\Data\262r\shards`
