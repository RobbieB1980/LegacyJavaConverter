...
public sealed class SetupForm : Form
{
    ...
    public SetupForm()
    {
        Text = "Install RB Legacy Java Converter";
        ...
        var title = new Label
        {
            Text = "RB Legacy Java Converter v1.2.0",
    ...
    private async Task InstallAsync()
    {
        ...
        try
        {
            ...
            var exe = Path.Combine(dest, "RB-Legacy-Java-Converter.exe");
            var r = MessageBox.Show(this,
                "Installed successfully to:\n" + dest + "\n\nLaunch RB Legacy Java Converter now?",
    ...
    private void InstallCore(string dest)
    {
        ...
        var exe = Path.Combine(dest, "RB-Legacy-Java-Converter.exe");
        if (!File.Exists(exe))
            throw new InvalidOperationException("Package extracted but RB-Legacy-Java-Converter.exe is missing.");
        ...
        if (_chkDesktop.Checked)
            CreateShortcut(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory),
                "RB Legacy Java Converter.lnk"), exe, dest);
        if (_chkStartMenu.Checked)
        {
            ...
            CreateShortcut(Path.Combine(sm, "RB Legacy Java Converter.lnk"), exe, dest);
        ...
        File.WriteAllText(Path.Combine(dest, "UNINSTALL.txt"),
            "To uninstall RB Legacy Java Converter:\r\n" +
        ...
        try
        {
            using var key = Registry.CurrentUser.CreateSubKey(
                @"Software\Microsoft\Windows\CurrentVersion\App Paths\RB-Legacy-Java-Converter.exe");
            key?.SetValue("", exe);
...
$s.Description = 'RB Legacy Java Converter'
$s.Save()
";
