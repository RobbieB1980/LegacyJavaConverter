# 262r — NeoForge 26.2 compile/runtime repair knowledge

Indexed remaps for **1.21.x (and Forge 1.20.1 residue) → NeoForge 26.2**.  
...
Proven on **CASE-005** `gecko_kings_avp_mod` and **Easy Mob Farm** NeoForge 1.21.4 → 26.2 (compile-green 2026-09-05). Compile-green is not play-green — encode leftovers here so Mode B does not rediscover them.

## Agent search order (mandatory)

1. Failed output packet: `GROK_REPAIR_PROMPT.md` → `MIGRATION_EVIDENCE` / `SOURCE_PROFILE` / `compile-errors`
2. **This tree (`262r`)** — `converter/` then matching `shards/` then `remaps.md` / `crashes.md`
...
6. Invent only if missing — then write back here **and** into the converter PS1

Do **not** start with a broad walk of all of `C:\gokuai\Data`.

## Read order

1. [pipeline.md](pipeline.md) — when 262-repair runs (and why it must re-run after MCreator).
2. [converter/](converter/) — dual-deps, import-detect, soft-dep exclude, Fix-in-Grok, capability stub.
3. [shards/](shards/) — open **one** family matching the current compiler/runtime error.
4. [remaps.md](remaps.md) — compile remaps ledger (target shape + detection).
5. [crashes.md](crashes.md) — in-game crash ladder after a green jar.
6. [do-not.md](do-not.md) — anti-patterns that regenerated the same errors.
...
| [map-entry-accessors.md](shards/map-entry-accessors.md) | `entry.property()` → `getKey()` |
| [entity-type-lookup.md](shards/entity-type-lookup.md) | `EntityType.byString` → BuiltInRegistries |
...
|---|---|---|
| constructMods | `Registry is already frozen` `minecraft:game_rule / …` | `mc-262r-gamerules-deferred` |
| RegisterEvent | `Block id not set` / `Item id not set` | `mc-262r-block-item-id` |
| World create | `Unbound tags … biome: [forge:is_cave]` | `mc-262r-convention-tags` |
