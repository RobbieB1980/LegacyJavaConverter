# Session continue — RB Legacy Java Converter 2.10.2

**Read this first** when reopening this worktree. Last saved: **2026-09-02**.

## Status (current)

- Product version: **2.10.2** (GUI Setup + Portable rebuilt with CASE-005 full-restore tools).
- Dist:
...
- AppData synced from portable tools + EXE.
- CASE-005 gecko_kings full restore jar ready; **not in-game verified** (Xbox services down).
...
| Dist artifacts | `C:\Users\rmbel\Documents\Codex\2026-08-29\referenced-chatgpt-conversation-this-is-an-4\work\LegacyJavaConverter\dist\` |
...
1. Install Setup or unzip Portable; confirm GUI title `v2.10.2`.
2. When Xbox is up: in-game verify CASE-005 jar on NeoForge 26.2.0.72.
3. Publish GitHub release only if desired (`Publish-GitHubRelease.ps1 -Tag v2.10.2`); `gh` auth was missing last session.
