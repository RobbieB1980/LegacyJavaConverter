<#
.SYNOPSIS
  Thin wrapper: launch station GrokBuild repair for a failed converter output.
.EXAMPLE
  .\Open-GrokRepairSession.ps1 -FailedOutput "C:\mods\mymod-26.2"
#>
param(
  [Parameter(Mandatory)][string]$FailedOutput,
  [string]$RbRoot = 'C:\rmblocal_llm',
  [string]$Workspace = 'C:\rmblocal_llm\projects\RB-Legacy-Java-Converter'
)
$ErrorActionPreference = 'Stop'
$start = Join-Path $RbRoot 'Start-GrokBuild.ps1'
...
& $start -ProjectPath $Workspace -Root $RbRoot -PromptFile $prompt
