...
function Write-GradleScaffold {
    param([string]$Root, [hashtable]$Meta, [string]$LocalLibs)
...
"@
    ...
    flatDir {
        dirs '$($LocalLibs.Replace('\', '/'))'
    }
"@
        # Prefer Maven first; flatDir as supplemental name lookup only if needed
    }
...
dependencies {
    // GeckoLib 5 for NeoForge 26.2
    implementation "com.geckolib:geckolib-neoforge-`${minecraft_version}:`${geckolib_version}"

    // SmartBrainLib 2.x for NeoForge 26.2 (coordinate may need adjustment)
...
Write-GradleScaffold -Root $OutputPath -Meta $meta -LocalLibs $LocalLibDir
...
if ($Compile) {
    Write-Step 'Running compileJava (expect errors - this is a diagnostic pass)'
    Push-Location $OutputPath
    try {
        & .\gradlew.bat compileJava --no-daemon --stacktrace 2>&1 | Tee-Object -FilePath (Join-Path $OutputPath 'compile-errors.log')
